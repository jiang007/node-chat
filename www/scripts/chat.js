...
Chat.prototype = {
  init: function () {
    ...
    self.socket.on("error", function (err) {
      if (document.getElementById('loginWrapper').stype.display == 'none') {
    ...
    self.socket.on('newMsg', function (user, msg, color) {
      self._displayNewMsg(user,msg,color);
    });

    self.socket.on('newImg', function (user, img, color) {
    ...
    document.getElementById('sendBtn').addEventListener('click', function (){
      ...
      if (msg.trim().length != 0) {
        self.socket.emit('postMsg', msg, color);
    ...
    document.getElementById('messageInput').addEventListener('keyup', function (e) {
      var messageInput = document.getElementById('messageInput'),
          msg = messageInput.value,
          color = document.getElementById('colorStyle').color;
